...
#define BIN(var, a0, nbins, min, max, ...)                                    \
   ...
   auto a##var##b = edges<n##var##b>(min, max, expand<n##var##b>{});          \
...
#define DEGTRIGGERSPT(ACTION, ...)                                            \
   if (maxPt2 > 10) { DEGMIN10TRIGGERS(ACTION, ## __VA_ARGS__)                \
      if (maxPt2 > 15) { DEGMIN15TRIGGERS(ACTION, ## __VA_ARGS__) } }
...
#define PAPER(l, t, n)                                                        \
   ...
   float max##l##t = hp##l##t->GetBinLowEdge(hp##l##t->GetNbinsX() + 1);      \
   TH1F* hfr##l##t = new TH1F("hfr" #l #t, "", 1, min##l##t, max##l##t);      \
   hfr##l##t->SetStats(0); hfr##l##t->SetAxisRange(0, 1.2, "Y");              \
   hfr##l##t->SetTitle(desc[#l].second.data()); hfr##l##t->Draw();            \
   ...
   TLegend* lg##l##t = new TLegend(0.32, l##t##y0, 0.9, l##t##y1);            \
   lg##l##t->SetFillStyle(0); lg##l##t->SetBorderSize(0);                     \
   lg##l##t->SetTextFont(43); lg##l##t->SetTextSize(12);

#define SAVE(label, tag)                                                      \
   c##label##tag->SaveAs(Form("figs/pdf/" #label "-" #tag "-%s.pdf", output));\
   c##label##tag->SaveAs(Form("figs/png/" #label "-" #tag "-%s.png", output));
...
#define AUTOYRANGE(label, TRIGGER)                                            \
   hfr##label##TRIGGER->SetAxisRange(0, label[#TRIGGER].first->GetBinContent( \
